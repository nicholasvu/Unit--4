...
# create table to hold school data (hashed out because table already made one you run the code once)
#with con:
...
# create table to hold gdp data (hashed out b/c table already exists)
#with con:
...
with open('/Users/nicholasvu/Desktop/Thinkful_Folder/Unit_3/ScrapeData/ny/ny.gdp.mktp.cd_Indicator_en_csv_v2.csv') as inputFile:
    next(inputFile)
    next(inputFile)
    header = next(inputFile)
    inputReader = csv.reader(inputFile)
